...
export class SlidingSyncCache {
    ...
    public forget(roomId: string): void {
        ...
        // Queue the IDB delete even when nothing was in memory: a record from an
        // earlier session may still be on disk (loadAll seeds `records`, so that
        // window is only pre-load), and deleting a missing key is a no-op.
