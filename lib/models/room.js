...
import { FILTER_RELATED_BY_REL_TYPES, FILTER_RELATED_BY_SENDERS, isThreadRelationEvent, Thread, THREAD_RELATION_TYPE, ThreadEvent, ThreadFilterType } from "./thread.js";
...
export class Room extends ReadReceipt {
  ...
  constructor(roomId, client, myUserId) {
    ...
    _defineProperty(this, "fullRosterFetched", false);
    ...
    _defineProperty(this, "backgroundBackfillPromise", void 0);
  ...
  getLastActiveTimestamp() {
    // Always a millisecond timestamp. Do NOT return the MSC4186 bump_stamp
    // here: per the spec it is an opaque per-server stream counter (small
    // integer, may even decrease), not a ms epoch. Returning it from a method
    // whose fallback branch yields ms would put callers' sorts on two
    // incomparable number spaces, slamming bump_stamp rooms to the bottom and
    // flickering them as stamps arrive. Consumers that want bump-aware
    // ordering should read getBumpStamp() and compare it ONLY against other
    // rooms' bump_stamps (see the Rust SDK's recency_stamp ordering).
  ...
  getDirectUserId() {
    var _this$client;
    var content = (_this$client = this.client) === null || _this$client === void 0 || (_this$client = _this$client.getAccountData(EventType.Direct)) === null || _this$client === void 0 ? void 0 : _this$client.getContent();
    if (!content) return null;
    for (var userId in content) {
      if (userId === this.myUserId) continue;
      var roomIds = content[userId];
  ...
  guessDMUserId() {
    ...
    var directUserId = this.getDirectUserId();
    if (directUserId) {
      return directUserId;
    }
    var me = this.getMember(this.myUserId);
    ...
    if (Array.isArray(this.heroes) && this.heroes.length) {
      ...
      var otherHero = (_this$heroes$find = this.heroes.find(h => h.userId !== this.myUserId)) !== null && _this$heroes$find !== void 0 ? _this$heroes$find : this.heroes[0];
  ...
  getAvatarFallbackMember() {
    ...
    var nonFunctionalHeroes = (_this$heroes2 = this.heroes) === null || _this$heroes2 === void 0 ? void 0 : _this$heroes2.filter(h => !functionalMembers.includes(h.userId) && h.userId !== this.myUserId);
    ...
    var directUserId = this.getDirectUserId();
    if (directUserId) {
      var directMember = this.getMember(directUserId);
  ...
  forceLoadMembers() {
    this.currentState.markOutOfBandMembersStarted();
    var update = this.loadMembersFromServer().then(rawMembers => {
      var memberEvents = rawMembers.filter(noUnsafeEventProps).map(this.client.getEventMapper());
      this.currentState.setOutOfBandMembers(memberEvents);
      this.fullRosterFetched = true;
      logger.log("LL: forceLoadMembers got ".concat(memberEvents.length, " members for room ").concat(this.roomId));
      var oobMembers = this.currentState.getMembers().filter(m => m.isOutOfBand()).map(m => {
        var _m$events$member2;
        return (_m$events$member2 = m.events.member) === null || _m$events$member2 === void 0 ? void 0 : _m$events$member2.event;
      });
      this.client.store.setOutOfBandMembers(this.roomId, oobMembers).catch(err => logger.log("LL: storing OOB room members failed, oh well", err));
      return true;
    }).catch(err => {
      // allow retries on fail
      this.membersPromise = undefined;
  ...
  /**
   * Re-arm out-of-band member loading so the next {@link loadMembersIfNeeded}
   * re-fetches the full membership from the server.
   *
   * Unlike {@link clearLoadedMembersIfNeeded} this does NOT drop the members
   * we already have — it only invalidates the "already loaded" latch — so
   * callers (notably the encryption path, via {@link getEncryptionTargetMembers})
   * re-read a complete roster that includes anyone who joined after the first
   * load. This is the JS analogue of the Rust SDK's `mark_members_missing()`.
   *
   * It closes a UTD hole under sliding sync: the lean `required_state` may
   * never deliver a late joiner's `m.room.member` event, so the cached roster
   * stays stale and the outbound megolm key is never shared to them. Calling
   * this when the server's `joined_count` exceeds the members we know forces a
   * fresh `/members` before the next key share.
   */
  invalidateLoadedMembers() {
    this.membersPromise = undefined;
  ...
  getRoomUnreadNotificationCount() {
    var _this$notificationCou, _this$timeline;
    var type = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : NotificationCountType.Total;
    var count = (_this$notificationCou = this.notificationCounts[type]) !== null && _this$notificationCou !== void 0 ? _this$notificationCou : 0;
    ...
    var latestId = (_this$timeline = this.timeline[this.timeline.length - 1]) === null || _this$timeline === void 0 ? void 0 : _this$timeline.getId();
    ...
    var readAtLatest = receiptType => {
      var _this$getReadReceiptF;
      return ((_this$getReadReceiptF = this.getReadReceiptForUserId(this.myUserId, true, receiptType)) === null || _this$getReadReceiptF === void 0 ? void 0 : _this$getReadReceiptF.eventId) === latestId;
    };
  ...
  backgroundBackfill(opts) {
    ...
    var liveTimeline = this.getLiveTimeline();
    var work = /*#__PURE__*/function () {
      var _ref3 = _asyncToGenerator(function* () {
        while (liveTimeline.getEvents().length < targetDepth) {
          if (abortSignal !== null && abortSignal !== void 0 && abortSignal.aborted) return;
          var token = liveTimeline.getPaginationToken(EventTimeline.BACKWARDS);
          if (!token) return;
          var moreAvailable = yield _this0.client.paginateEventTimeline(liveTimeline, {
            backwards: true,
            limit: chunkSize,
  ...
  getEncryptionTargetMembers() {
    var _this1 = this;
    return _asyncToGenerator(function* () {
      var _this1$client$getSlid, _this1$client;
      ...
      if (!_this1.fullRosterFetched && (_this1$client$getSlid = (_this1$client = _this1.client).getSlidingSync) !== null && _this1$client$getSlid !== void 0 && _this1$client$getSlid.call(_this1$client)) {
        yield _this1.forceLoadMembers();
      }
      yield _this1.loadMembersIfNeeded();
      var members = _this1.getMembersWithMembership(KnownMembership.Join);
      if (_this1.shouldEncryptForInvitedMembers()) {
        members = members.concat(_this1.getMembersWithMembership(KnownMembership.Invite));
      }
      return members;
    })();
  }
  ...
  getThreadListFilter() {
    ...
    return _asyncToGenerator(function* () {
      ...
      var myUserId = _this10.client.getUserId();
      ...
      var filterId = yield _this10.client.getOrCreateFilter("THREAD_PANEL_".concat(_this10.roomId, "_").concat(filterType), filter);
  ...
  createThreadTimelineSet(filterType) {
    ...
    return _asyncToGenerator(function* () {
      ...
      if (Thread.hasServerSideListSupport) {
        timelineSet = new EventTimelineSet(_this11, _objectSpread(_objectSpread({}, _this11.opts), {}, {
          pendingEvents: false
        }), undefined, undefined, filterType !== null && filterType !== void 0 ? filterType : ThreadFilterType.All);
        _this11.reEmitter.reEmit(timelineSet, [RoomEvent.Timeline, RoomEvent.TimelineReset]);
      } else if (Thread.hasServerSideSupport) {
        var filter = yield _this11.getThreadListFilter(filterType);
        timelineSet = _this11.getOrCreateFilteredTimelineSet(filter, {
          prepopulateTimeline: false,
          useSyncEvents: false,
          pendingEvents: false
        });
      } else {
        timelineSet = new EventTimelineSet(_this11, {
          pendingEvents: false
        });
        Array.from(_this11.threads).forEach(_ref4 => {
          var [, thread] = _ref4;
          if (thread.length === 0) return;
          var currentUserParticipated = thread.timeline.some(event => {
            return event.getSender() === _this11.client.getUserId();
          });
  ...
  fetchRoomThreads() {
    var _this12 = this;
    return _asyncToGenerator(function* () {
      var _this12$client$isFull, _this12$client;
      if (_this12.threadsReady || !_this12.client.supportsThreads()) {
        return;
      }
      if (Thread.hasServerSideListSupport) {
        yield Promise.all([_this12.fetchRoomThreadList(ThreadFilterType.All), _this12.fetchRoomThreadList(ThreadFilterType.My)]);
      } else if ((_this12$client$isFull = (_this12$client = _this12.client).isFullLazyLoading) !== null && _this12$client$isFull !== void 0 && _this12$client$isFull.call(_this12$client)) {
        ...
        var allThreadsFilter = yield _this12.getThreadListFilter();
        var {
          chunk: events
        } = yield _this12.client.createMessagesRequest(_this12.roomId, "", Number.MAX_SAFE_INTEGER, Direction.Backward, allThreadsFilter);
        if (!events.length) return;

        // Sorted by last_reply origin_server_ts
        var threadRoots = events.map(_this12.client.getEventMapper()).sort((eventA, eventB) => {
          /**
        ...
        var roomState = _this12.getLiveTimeline().getState(EventTimeline.FORWARDS);
        for (var rootEvent of threadRoots) {
          var _this12$threadsTimeli;
          var _opts = {
          ...
          (_this12$threadsTimeli = _this12.threadsTimelineSets[0]) === null || _this12$threadsTimeli === void 0 || _this12$threadsTimeli.addLiveEvent(rootEvent, _opts);
          ...
          if (threadRelationship !== null && threadRelationship !== void 0 && threadRelationship.current_user_participated) {
            var _this12$threadsTimeli2;
            (_this12$threadsTimeli2 = _this12.threadsTimelineSets[1]) === null || _this12$threadsTimeli2 === void 0 || _this12$threadsTimeli2.addLiveEvent(rootEvent, _opts);
            latestMyThreadsRootEvent = rootEvent;
          }
        }
        _this12.processThreadRoots(threadRoots, true);
        _this12.client.decryptEventIfNeeded(threadRoots[threadRoots.length - 1]);
        if (latestMyThreadsRootEvent) {
          _this12.client.decryptEventIfNeeded(latestMyThreadsRootEvent);
        }
      }
      _this12.on(ThreadEvent.NewReply, _this12.onThreadReply);
      _this12.on(ThreadEvent.Update, _this12.onThreadUpdate);
      _this12.on(ThreadEvent.Delete, _this12.onThreadDelete);
      ...
      var localThreadCandidates = _this12.getUnfilteredTimelineSet().getLiveTimeline().getEvents().filter(e => !e.isRedacted() && isThreadRelationEvent(e));
      if (localThreadCandidates.length > 0) {
        _this12.processThreadedEvents(localThreadCandidates, false);
      }
      ...
      _this12.backfillOrphanedThreadRoots();
      _this12.threadsReady = true;
    })();
  }
    ...
    return _asyncToGenerator(function* () {
      for (var event of events) {
        try {
           ...
           */
          yield _this13.client.decryptEventIfNeeded(event);
          _this13.processPollEvent(event);
        } catch (err) {
    ...
    return _asyncToGenerator(function* () {
      if (event.isDecryptionFailure()) {
        event.once(MatrixEventEvent.Decrypted, maybeDecryptedEvent => {
          _this14.processPollEvent(maybeDecryptedEvent);
        });
        return;
      }
      if (M_POLL_START.matches(event.getType())) {
        try {
          var poll = new Poll(event, _this14.client, _this14);
          _this14.polls.set(event.getId(), poll);
          _this14.emit(PollEvent.New, poll);

          // remove the poll when redacted
          event.once(MatrixEventEvent.BeforeRedaction, redactedEvent => {
            _this14.polls.delete(redactedEvent.getId());
          });
        } catch (_unused3) {}
      ...
      if (relationEventId && _this14.polls.has(relationEventId)) {
        var _poll = _this14.polls.get(relationEventId);
  ...
  fetchRoomThreadList(filter) {
    var _this15 = this;
    return _asyncToGenerator(function* () {
      if (!_this15.client.supportsThreads()) return;
      if (_this15.threadsTimelineSets.length === 0) return;
      var timelineSet = filter === ThreadFilterType.My ? _this15.threadsTimelineSets[1] : _this15.threadsTimelineSets[0];
      var {
        chunk: events,
        end
      } = yield _this15.client.createThreadListMessagesRequest(_this15.roomId, null, undefined, Direction.Backward, timelineSet.threadListType, timelineSet.getFilter());
      timelineSet.getLiveTimeline().setPaginationToken(end !== null && end !== void 0 ? end : null, Direction.Backward);
      if (!events.length) return;
      var matrixEvents = events.map(_this15.client.getEventMapper());
      _this15.processThreadRoots(matrixEvents, true);
      var roomState = _this15.getLiveTimeline().getState(EventTimeline.FORWARDS);
  ...
  eventShouldLiveIn(event, events, roots) {
    ...
    var isThreadRelation = isThreadRelationEvent(event);
  ...
  processThreadedEvents(events, toStartOfTimeline) {
    ...
    Object.entries(eventsByThread).map(_ref5 => {
   ...
   */
  maybeBackfillThreadRoots(events) {
    for (var event of events) {
      var eventId = event.getId();
      if (eventId === undefined) continue;
      var orphanedThread = this.threads.get(eventId);
   ...
   */
  backfillOrphanedThreadRoots() {
    for (var thread of this.threads.values()) {
      if (thread.rootEvent) continue;
      var rootEvent = this.findEventById(thread.id);
      if (rootEvent) {
  ...
  addLiveEvents(events, addLiveEventOptions) {
    ...
    return _asyncToGenerator(function* () {
      ...
      _this16.assertTimelineSetsAreLive();
      var threadRoots = _this16.findThreadRoots(events);
      var eventsByThread = {};
      var options = {
        duplicateStrategy,
        fromCache,
      ...
      for (var event of events) {
        ...
        _this16.processLiveEvent(event);
        if (event.getUnsigned().transaction_id) {
          var existingEvent = _this16.txnToEvent.get(event.getUnsigned().transaction_id);
          if (existingEvent) {
            // remote echo of an event we sent earlier
            _this16.handleRemoteEcho(event, existingEvent);
        ...
        var {
          ...
          shouldLiveInThread,
          threadId = ""
        } = _this16.eventShouldLiveIn(event, neighbouringEvents, threadRoots);
        if (!shouldLiveInThread && !shouldLiveInRoom && event.isRelation()) {
          try {
            var parentEvent = new MatrixEvent(yield _this16.client.fetchRoomEvent(_this16.roomId, event.relationEventId));
        ...
        if (shouldLiveInRoom) {
          _this16.addLiveEvent(event, options);
        } else if (!shouldLiveInThread && event.isRelation()) {
          _this16.relations.aggregateChildEvent(event);
        }
      }
      Object.entries(eventsByThread).forEach(_ref6 => {
        var [threadId, threadEvents] = _ref6;
        _this16.addThreadedEvents(threadId, threadEvents, false);
      });
    })();
  }
        ...
        Object.keys(content[eventId][receiptType]).forEach(userId => {
          ...
          if (receiptDestination) {
            ...
            if (!synthetic && this.client.isInitialSyncComplete() && userId === this.client.getUserId()) {
              ...
              // If OUR read receipt is at the last event, we've read everything in
              // this room — clear the unread counts without waiting for the server
              // to echo a fresh count. Previously this also required us to be the
              // SENDER of that last event ("read your own message"), which is too
              // narrow: under sliding sync a room read elsewhere can carry a stale
              // server notification_count, and our receipt landing on the last event
              // is proof enough that it's read. (We keep the synthetic + initial-sync
              // guards so this only runs on a genuine, post-startup receipt.)
              if (lastEvent && eventId === lastEvent.getId()) {
  ...
  calculateRoomName(userId) {
    ...
    if (this.heroes) {
      ...
      otherMembers = otherMembers.filter(_ref7 => {
        var {
          userId
        } = _ref7;
        // filter service members
