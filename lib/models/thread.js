...
export class Thread extends ReadReceipt {
  constructor(id, rootEvent, opts) {
    ...
    _defineProperty(this, "onBeforeRedaction", (event, redaction) => {
      if (isThreadRelationEvent(event) && this.room.eventShouldLiveIn(event).threadId === this.id && event.getId() !== this.id &&
  ...
  addEvent(event, toStartOfTimeline) {
    ...
    if (event.getId() !== this.id && isThreadRelationEvent(event) && !toStartOfTimeline && isNewestReply) {
  ...
  updatePendingReplyCount() {
    ...
    var pendingEvents = unfilteredPendingEvents.filter(ev => {
      ...
      return ev.threadRootId === this.id && isThreadRelationEvent(ev) && ev.status !== null && ev.getId() !== ((_this$lastEvent2 = this.lastEvent) === null || _this$lastEvent2 === void 0 ? void 0 : _this$lastEvent2.getId());
    });
  ...
  updateThreadFromRootEvent() {
    var _this6 = this;
    return _asyncToGenerator(function* () {
      var _this6$client$isFullL, _this6$client;
      ...
      if (Thread.hasServerSideSupport || (_this6$client$isFullL = (_this6$client = _this6.client).isFullLazyLoading) !== null && _this6$client$isFullL !== void 0 && _this6$client$isFullL.call(_this6$client)) {
   ...
   */
  lastReply() {
    var matches = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : ev => isThreadRelationEvent(ev);
 ...
 */
export function isThreadRelationEvent(event) {
  var _event$getRelation;
  var relType = (_event$getRelation = event.getRelation()) === null || _event$getRelation === void 0 ? void 0 : _event$getRelation.rel_type;
