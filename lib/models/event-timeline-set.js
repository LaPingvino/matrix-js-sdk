...
export class EventTimelineSet extends TypedEventEmitter {
   ...
   *
   */
  addEventsToTimeline(events, toStartOfTimeline, addToState, timeline, paginationToken, quiet) {
    if (!timeline) {
  ...
  addEventToTimeline(event, timeline, _ref2) {
    ...
    if (this.room && !this.canContain(event)) {
      ...
      logger.debug("EventTimelineSet.addEventToTimeline: Ignoring ".concat(eventDebugString, " that does not belong ") + "in timeline=".concat(timeline.toString(), " timelineSet(threadId=").concat((_this$thread2 = this.thread) === null || _this$thread2 === void 0 ? void 0 : _this$thread2.id, ")"));
      return;
    }
  ...
  canContain(event) {
    ...
    if (!shouldLiveInRoom && !shouldLiveInThread) {
      var _this$room2;
      logger.debug("EventTimelineSet:canContain event encountered which cannot be added to any timeline roomId=".concat((_this$room2 = this.room) === null || _this$room2 === void 0 ? void 0 : _this$room2.roomId, " eventId=").concat(event.getId(), " threadId=").concat(event.threadRootId));
    }
