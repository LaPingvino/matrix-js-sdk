...
export class ReadReceipt extends TypedEventEmitter {
   ...
   */
  wouldRegressReceipt(userId, event, receiptType) {
    var candidateId = event.getId();
    if (!candidateId) return false;
    var existing = this.getReadReceiptForUserId(userId, false, receiptType);
    if (!existing) return false;
    if (existing.eventId === candidateId) return true; // same position — redundant
    var ordering = this.getUnfilteredTimelineSet().compareEventOrdering(existing.eventId, candidateId);
    ...
    var existingEvent = this.findEventById(existing.eventId);
