...
function ensureNameEvent(client, roomId, roomData) {
  ...
  var heroName = dmNameFromHeroes(roomData);
 ...
 */
function dmNameFromHeroes(roomData) {
  ...
  var names = heroes.filter(h => !functional.has(h.user_id)).map(h => h.displayname || h.user_id);
