...
var ENCRYPTION_SYNC_TIMEOUT_MS = 3000;
...
export class SlidingSyncSdk {
    ...
    return _asyncToGenerator(function* () {
      try {
        ...
        if (!_this8.rehydrating) {
          ...
          _this8.roomCache.put(roomId, roomData, receipt);
        }
      } catch (e) {
  ...
  processRoomData(client, room, roomData) {
    var _this9 = this;
    return _asyncToGenerator(function* () {
      var _selfMember$getConten, _ref;
      ...
      room.updateMyMembership((_ref = selfMembership) !== null && _ref !== void 0 ? _ref : KnownMembership.Join);
  ...
  rehydrateFromCache() {
    ...
    return _asyncToGenerator(function* () {
      var rooms;
      try {
        rooms = yield _this1.roomCache.loadAll();
      } catch (e) {
        _this1.syncOpts.logger.warn("[sss-cache] rehydrate load failed; cold start", e);
        return;
      }
      if (rooms.length === 0) return;
      _this1.syncOpts.logger.debug("[sss-cache] rehydrating ".concat(rooms.length, " rooms from cache"));
      _this1.rehydrating = true;
      try {
        for (var {
          roomId,
          data,
          ...
          yield _this1.onRoomData(roomId, data);
          ...
          if (receipt) {
            try {
              processEphemeralEvents(_this1.client, roomId, [receipt]);
            } catch (e) {
              _this1.syncOpts.logger.debug("[sss-cache] receipt replay failed", e);
            }
          }
        }
      } catch (e) {
        _this1.syncOpts.logger.warn("[sss-cache] rehydrate replay failed", e);
      } finally {
        _this1.rehydrating = false;
      }
    })();
  }
  ...
  sync() {
    var _this10 = this;
    return _asyncToGenerator(function* () {
      var _this10$encryptionSyn;
      _this10.syncOpts.logger.debug("Sliding sync init loop");

      //   1) We need to get push rules so we can check if events should bing as we get
      //      them from /sync.
      while (!_this10.client.isGuest()) {
        try {
          _this10.syncOpts.logger.debug("Getting push rules...");
          var result = yield _this10.client.getPushRules();
          _this10.syncOpts.logger.debug("Got push rules");
          _this10.client.pushRules = result;
          break;
        } catch (err) {
          _this10.syncOpts.logger.error("Getting push rules failed", err);
          if (_this10.shouldAbortSync(err)) {
            return;
          }
        }
      }
      ...
      yield _this10.rehydrateFromCache();
      ...
      void Promise.all([EventType.Direct, EventType.IgnoredUserList, EventType.PushRules].map(/*#__PURE__*/function () {
        var _ref3 = _asyncToGenerator(function* (type) {
          try {
            var content = yield _this10.client.getAccountDataFromServer(type);
            if (!content) return;
            var [ev] = mapEvents(_this10.client, undefined, [{
              type,
              content
            }]);
            if (!ev) return;
            var prev = _this10.client.store.getAccountData(type);
            _this10.client.store.storeAccountDataEvents([ev]);
            _this10.client.emit(ClientEvent.AccountData, ev, prev);
            ...
            if (type === EventType.Direct && content && typeof content === "object") {
              ...
              if (dmRoomIds.size) {
                ...
                _this10.slidingSync.addCustomSubscription(DM_MATERIALIZE_SUB, {
                  timeline_limit: 1,
                  required_state: DEFAULT_SLIDING_SYNC_REQUIRED_STATE
                });
                var subs = _this10.slidingSync.getRoomSubscriptions();
                var added = false;
                for (var id of dmRoomIds) {
                  if (!subs.has(id)) {
                    subs.add(id);
                    _this10.slidingSync.useCustomSubscription(id, DM_MATERIALIZE_SUB);
                    added = true;
                  }
                }
                if (added) _this10.slidingSync.modifyRoomSubscriptions(subs);
              }
            }
          } catch (_unused) {
      ...
      (_this10$encryptionSyn = _this10.encryptionSync) === null || _this10$encryptionSyn === void 0 || _this10$encryptionSyn.start().catch(e => _this10.syncOpts.logger.error("encryption sliding sync failed", e));
      yield _this10.slidingSync.start();
    })();
  }
