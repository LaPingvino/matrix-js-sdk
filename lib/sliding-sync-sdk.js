    ...
    return _asyncToGenerator(function* () {
      ...
      void Promise.all([EventType.Direct, EventType.IgnoredUserList, EventType.PushRules].map(/*#__PURE__*/function () {
        var _ref3 = _asyncToGenerator(function* (type) {
          try {
            var content = yield _this1.client.getAccountDataFromServer(type);
            if (!content) return;
            var [ev] = mapEvents(_this1.client, undefined, [{
              type,
              content
            }]);
            if (!ev) return;
            var prev = _this1.client.store.getAccountData(type);
            _this1.client.store.storeAccountDataEvents([ev]);
            _this1.client.emit(ClientEvent.AccountData, ev, prev);
            ...
            if (type === EventType.Direct && content && typeof content === "object") {
              ...
              if (dmRoomIds.size) {
                ...
                _this1.slidingSync.addCustomSubscription(DM_MATERIALIZE_SUB, {
                  timeline_limit: 1,
                  required_state: DEFAULT_SLIDING_SYNC_REQUIRED_STATE
                });
                var subs = _this1.slidingSync.getRoomSubscriptions();
                var added = false;
                for (var id of dmRoomIds) {
                  if (!subs.has(id)) {
                    subs.add(id);
                    _this1.slidingSync.useCustomSubscription(id, DM_MATERIALIZE_SUB);
                    added = true;
                  }
                }
                if (added) _this1.slidingSync.modifyRoomSubscriptions(subs);
              }
            }
          } catch (_unused) {
      ...
      (_this1$encryptionSync = _this1.encryptionSync) === null || _this1$encryptionSync === void 0 || _this1$encryptionSync.start().catch(e => _this1.syncOpts.logger.error("encryption sliding sync failed", e));
