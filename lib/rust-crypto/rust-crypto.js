...
import { AllDevicesIsolationMode, CrossSigningKey, CryptoEvent, DecryptionFailureCode, deriveRecoveryKeyFromPassphrase, DeviceIsolationModeKind, DeviceVerificationStatus, encodeRecoveryKey, EventShieldColour, EventShieldReason, ImportRoomKeyStage, UserVerificationStatus, VerificationPhase } from "../crypto-api/index.js";
...
export class RustCrypto extends TypedEventEmitter {
   ...
   */
  cancelStaleToDeviceVerifications(keepTransactionId) {
    var _this31 = this;
    return _asyncToGenerator(function* () {
      var requests = _this31.getVerificationRequestsToDeviceInProgress(_this31.userId);
      for (var request of requests) {
        if (request.transactionId !== keepTransactionId && request.phase !== VerificationPhase.Done && request.phase !== VerificationPhase.Cancelled) {
          try {
            yield request.cancel();
          } catch (e) {
            _this31.logger.warn("Failed to cancel stale verification request ".concat(request.transactionId), e);
          }
        }
      }
    })();
  ...
  requestOwnUserVerification() {
    ...
    return _asyncToGenerator(function* () {
      ...
      yield _this34.cancelStaleToDeviceVerifications();
      try {
  ...
  requestDeviceVerification(userId, deviceId) {
    ...
    return _asyncToGenerator(function* () {
      ...
      // Clean slate: never let a stale/abandoned flow coexist with the fresh one.
      yield _this35.cancelStaleToDeviceVerifications();
      try {
  ...
  onIncomingKeyVerificationRequest(sender, transactionId) {
    ...
    if (request) {
      ...
      void this.cancelStaleToDeviceVerifications(transactionId).catch(e => this.logger.warn("Failed to sweep stale verifications on incoming request", e));
