...
class BaseRustVerifer extends TypedEventEmitter {
  ...
  onChange() {
    if (this.inner.isDone()) {
      ...
      // eslint-disable-next-line no-console
      console.warn("[sdk][verify] verifier onChange: CANCELLED by ".concat(cancelInfo.cancelledbyUs() ? "US" : "THEM", " code=").concat(cancelInfo.cancelCode(), " reason=").concat(cancelInfo.reason()));
      ...
      this.callbacks = {
        ...
        confirm: function () {
          var _confirm = _asyncToGenerator(function* () {
            ...
            // eslint-disable-next-line no-console
            console.warn("[sdk][verify] confirm() produced ".concat(requests.length, " outgoing request(s)"));
