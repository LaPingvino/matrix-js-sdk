    ...
    return _asyncToGenerator(function* () {
      ...
      yield promiseMapSeries(joinRooms, /*#__PURE__*/function () {
        var _ref8 = _asyncToGenerator(function* (joinObj) {
          ...
          // Resilience: one bad room must not abort the whole join batch (see src/sync.ts).
          try {
          ...
          room.decryptCriticalEvents();
            ...
            _this9.syncOpts.logger.error("Failed to process joined room " + ((room && room.roomId) || "?") + " during sync; skipping it", e);
          }
        });
        return function (_x4) {
